...
lines = set(chain.from_iterable(codecs.open(f, 'r',encoding="utf-8") for f in glob('../data/file/machine_learning.txt')))
lines = [line.lower() for line in lines]
with codecs.open('../data/file/machine_learning-lower.txt', 'w',encoding="utf-8") as out:
    out.writelines(sorted(lines))
sentences = word2vec.Text8Corpus('../data/file/machine_learning-lower.txt')
model = word2vec.Word2Vec(sentences, size=200, workers=12, min_count=3, sg=0, window=8, iter=15, sample=1e-4, negative=25)
